...
KnightPlayer::KnightPlayer(int lev)
	: NovicePlayer(lev)
{
	attack = 40 + 10 * lev;
	defense = 20 + 12 * lev;
...
KnightPlayer::KnightPlayer(int lev, string nam)
	: NovicePlayer(lev, nam)
{
	attack = 40 + 10 * lev;
	defense = 20 + 12 * lev;
	max_hp = 150 + 25 * lev;
...
KnightPlayer::KnightPlayer(const KnightPlayer& copy)
	: NovicePlayer(copy)
{
	attack = 40 + 10 * level;
	defense = 20 + 12 * level;
...
void KnightPlayer::specialSkill() // heal(): increasing HP (level*10) points by decreasing MP (level*5) points
{
