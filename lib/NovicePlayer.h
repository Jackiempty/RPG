...
class NovicePlayer {
 public:
  ...
  NovicePlayer(int);  // normal constructor, Initializes level of the player,
                      // name = ��anonymous��
  NovicePlayer(
      int,
      string);  // normal constructor, Initializes level and name of the player
  NovicePlayer(const NovicePlayer&);  // Copy constructor
  // Getter/setters of name, level, hp, mp, exp, money
  void setName(string);
  string getName(void) const;
  virtual void setLevel(
      int);  //  Please note that you should also calculate attack, defense,
             //  max_hp, max_mp and lvup_exp while calling setLevel()
  ...
  Please also check other limits on data members and validate values within
  setters. For attack, defense, max_hp, max_mp and lvup_exp : Should be
  calculated automatically while initializing and calling setLevel() So no
  general setters for these members You can just ignore the procedures while
  leveling-up, we will deal with it in future classes. Or you can deal with it
  within setExp() if you want.
  */
  ...
  //	void countweight(); // used to refresh the current weight of the
  //backpack
