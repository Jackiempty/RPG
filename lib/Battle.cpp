        ...
        switch (AttackOrder[i]->type) {
          case ('p'):
            ...
            for (int i = 0; i < tmp_player->Backpack.size(); i++) {
              if (tmp_player->Backpack[i] != nullptr) {
                if (tmp_player->Backpack[i]->type == 'c') {
                  Items.push_back(
                      static_cast<ConsumableItem *>(tmp_player->Backpack[i]));
                }
              }
            }
            ...
            while (true) {
              cin >> movechoice;
              if (cin.fail() ||
                  (movechoice != 1 && movechoice != 2 && movechoice != 3)) {
                cin.clear();
                cin.ignore(100, '\n');
                cout << "Please input number \"1\", \"2\" or \"3\" " << endl;
              } else if (movechoice == 3 && Items.size() == 0) {
                cout << "There's no item to be used, please make another choice"
                     << endl;
              } else {
                break;
              }
            ...
            if (movechoice == 1) {
              cout << "Please choose a monster to attack, by entering the "
              ...
              if (tmp_player->Weapon == nullptr) {
                ...
                atk = tmp_player->getAttack() +
                      tmp_player->Weapon->attack_increment;
              }
              if (atk <
                  MonsterList[choice]->getHP() + MonsterList[choice]->defense) {
                cout << MonsterList[choice]->name << " is attacked" << endl;
                sleep(1);
                int nowmhp = MonsterList[choice]->getHP();
                if (atk <= MonsterList[choice]->defense) {
                  cout << "Player didn't break the defence of the monster"
                       << endl;
                  sleep(1);
                } else {
                  MonsterList[choice]->setHP(nowmhp - atk +
                                             MonsterList[choice]->defense);
                }
              } else {
                cout << MonsterList[choice]->name << " is attacked, and killed"
                     << endl;
                sleep(1);
          ...
          case ('m'):
            ...
            if (PlayerList[atkchoice]->Armor == nullptr) {
              ...
              plyrdefense = PlayerList[atkchoice]->getDefense() +
                            PlayerList[atkchoice]->Armor->defense_increment;
            }
            if (atk < PlayerList[atkchoice]->getHP() + plyrdefense) {
              cout << PlayerList[atkchoice]->getName() << " is attacked"
                   << endl;
              sleep(1);
              int nowmhp = PlayerList[atkchoice]->getHP();
              if (atk <= plyrdefense) {
                cout << "Monster's attack didn't break the player's defense"
                     << endl;
                sleep(1);
              } else {
              ...
              cout << PlayerList[atkchoice]->getName()
                   << " is attacked, and killed" << endl;
              sleep(1);
