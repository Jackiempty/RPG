...
int Field::getMapSymbol(int y, int x)
{
	return map_data[y][x];
}
...
bool Field::Trigger(NovicePlayer* plyr)
{
	if (map_data[current_position_y][current_position_x] == 2) // event
	{
		...
		sleep(1);
		system("clear");
		cout << "You have aquired a sword, a tunic and a lifepotion" << endl;
		Item* weapon1= new SwordWeapon;
		Item* armor1 = new TunicArmor;
		Item* potion1 = new LifePotion;
		plyr->putItem(weapon1);
		plyr->putItem(armor1);
	...
	else if (map_data[current_position_y][current_position_x] == 9) // teleport 
	{
