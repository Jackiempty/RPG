...
bool Game::Trigger() {
  if (CurrentMap->getMapSymbol(CurrentMap->getCurrentPositionY(),
                               CurrentMap->getCurrentPositionX()) ==
      2)  // event
  {
    system("clear");
    ...
    //		cout << "Here?" << endl;
    CurrentMap->setMapSymbol(0, CurrentMap->getCurrentPositionY(),
                             CurrentMap->getCurrentPositionX());
    getchar();  // replacement for system("pause")
    return true;
  } else if (CurrentMap->getMapSymbol(CurrentMap->getCurrentPositionY(),
                                      CurrentMap->getCurrentPositionX()) ==
             4)  // battle
  {
    Battle battle(1, 2);
    ...
    if (battle.Fight() == true) {
      ...
      getchar();  // replacement for system("pause")
      PlayerList[0]->setExp(PlayerList[0]->getExp() + monster1.exp +
                            monster2.exp);
      PlayerList[0]->setMoney(PlayerList[0]->getMoney() + monster1.money +
                              monster2.money);
      PlayerList[1]->setExp(PlayerList[1]->getExp() + monster1.exp +
                            monster2.exp);
      PlayerList[1]->setMoney(PlayerList[1]->getMoney() + monster1.money +
                              monster2.money);
             ...
             6)  // shop
  {
    ...
    return true;
             ...
             7)  // end
  {
    cout << "You have passed the final exam, and cleared this game, hope you "
            "had a good time"
         << endl;
    sleep(1);
...
//		cout << PlayerList[index] << endl;
    cout << tmp_plyr->getName() << "'s backpack:  (press Z to switch)" << endl
<< endl;
//		cout << "Current backpack weight: " << tmp_plyr->backpack_weight
<< "/" << tmp_plyr->backpack_weight_limit << endl; if (tmp_plyr->Weapon ==
nullptr)
    {
    ...
    {
            cout << "Total Attack: " << tmp_plyr->getAttack() +
tmp_plyr->Weapon->attack_increment << endl;
    }
    ...
    {
            cout << "Total Defence: " << tmp_plyr->getDefense() +
tmp_plyr->Armor->defense_increment << endl;
    }
...
void Game::openbox() {
  if (CurrentMap->getMapName() == "map01.txt") {
    ...
    if (CurrentMap->getCurrentPositionY() == 1 &&
        CurrentMap->getCurrentPositionX() == 1) {
      // CurrentMap = Maps[0];
    } else if (CurrentMap->getCurrentPositionY() == 10 &&
               CurrentMap->getCurrentPositionX() == 10) {
      // CurrentMap = Maps[2];
    }
...
void Game::Shop() {
  ...
  while (true) {
    ...
    system("clear");
    cout << "It is " << tmp_plyr->getName() << " buying (press Z to switch)"
         << endl;
...
void Game::Menu(bool& main) {
  ...
  while (true) {
    ...
    cout << "1:  " << "Switch player (" << PlayerList[0]->getName() << ")"
         << endl;
...
void Game::Save() {
  ...
  out3 << CurrentMap->getCurrentPositionY() << " "
       << CurrentMap->getCurrentPositionX() << endl;
  //	out3 << "shujinnkou:\n";
...
void Game::Main(bool& main, bool& quit) {
  ...
  while (true) {
    ...
    if (move == '3') {
      cout << "Are you sure you wanna quit? (If you didn't save, you will lose "
              "your progress)"
           << endl;
...
void Game::story() {
  if (stage == 0) {
    //	cout << "This is a story of you trying the pass the final exams which
    //are all hard as hell... " << endl; 	sleep(5); 	cout << "Try to clear all the
    //challenges on the way to your lovely summer vocation!" << endl; 	sleep(5);
  } else if (stage == 1) {
    system("clear");
    //	cout << "This is the maze of the professors' inner mind, once you go
    //through it, you realize what professors are thinking" << endl; 	sleep(3);
    stage++;
  } else if (stage == 2) {
    system("clear");
    //	cout << "This is your final step to pass all the exams, with all the
    //assistance I gave you" << endl
