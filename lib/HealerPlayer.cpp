...
HealerPlayer::HealerPlayer(int lev)
	: NovicePlayer(lev)
{
	attack = 30 + 8 * lev;
	defense = 20 + 7 * lev;
...
HealerPlayer::HealerPlayer(int lev, string nam)
...
HealerPlayer::HealerPlayer(const HealerPlayer& copy)
	: NovicePlayer(copy)
{
	attack = 30 + 8 * level;
	defense = 20 + 7 * level;
...
void HealerPlayer::specialSkill() // pray(): increasing MP (level*10) points by decreasing HP (level*5) points
{
