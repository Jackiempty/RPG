...
string NovicePlayer::getName(void) const { return name; }
...
int NovicePlayer::getLevel(void) const { return level; }
...
int NovicePlayer::getHP(void) const { return hp; }
...
int NovicePlayer::getMP(void) const { return mp; }
...
int NovicePlayer::getExp(void) const { return exp; }
...
int NovicePlayer::getMoney(void) const { return money; }

int NovicePlayer::getAttack(void) const { return attack; }

int NovicePlayer::getDefense(void) const { return defense; }

int NovicePlayer::getMaxHP(void) const { return max_hp; }

int NovicePlayer::getMaxMP(void) const { return max_mp; }

int NovicePlayer::getLvupExp(void) const { return lvup_exp; }
...
int NovicePlayer::getInstanceCount() { return count; }
...
void NovicePlayer::putItem(Item* item) {
  ...
  if (backpack_weight + itemweight <= backpack_weight_limit)
  {
          ...
          cout << "*Your backpack is full, please try again when it has space*"
  << endl; return false;
  }
  else
  {
          cout << "*Your backpack is too heavy, you can't put this item into the
  backpack*" << endl; usleep(500000); return false;
  }
  */
}
...
Item* NovicePlayer::takeItem(int index) {
  ...
  cout << Backpack[index]->name << " is taken from " << name << "'s backpack"
       << endl;
  sleep(1);
...
void NovicePlayer::Viewbackpack() {
  ...
  //	cout << "Current backpack weight: " << backpack_weight << "/" <<
  //backpack_weight_limit << endl;
  if (Weapon == nullptr) {
  ...
  switch (move) {
    case ('q'):
      ...
      if (Backpack[index] == nullptr) {
        ...
        //			cout << "Item pointer location: " <<
        //Backpack[index] << endl; 			getchar(); // replacement for system("pause")
